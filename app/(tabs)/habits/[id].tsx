...
import { View, Text, useColorScheme, SafeAreaView, ScrollView, ActivityIndicator, Image } from 'react-native'
...
export default function HabitScreen() {
  const { id, name, description, frequency, planned_time, notify, total_points } = useLocalSearchParams()
      ...
      <ScrollView>
        <ThemedView className="flex-1 pb-20">
          ...
          >
            ...
            >
              You planned this {frequency} for <Text className="text-lime-500">{planned_time}</Text> minutes.
            </ThemedText>
            <ThemedText className="text-sm font-pregular">
              {total_points && Number(total_points) > 0 ? `Total points: ${total_points}` : null}
            </ThemedText>
            <ThemedText className="text-sm font-pregular">
              Current streak: {activitySummary?.currentStreak} | Max streak: {activitySummary?.maxStreak}
            </ThemedText>
          </View>
          <ScrollView horizontal>
            ...
            ) : activitySummary && activitySummary?.data?.length > 0 ? (
              <View className="h-60 max-h-96 w-96 mx-auto overflow-y-hidden overflow-x-auto">
                <ContributionGraph
                  values={activitySummary?.data}
                  endDate={new Date(activitySummary?.data[0]?.date)}
                  numDays={105}
                  width={500}
                  height={220}
                  gutterSize={2}
          ...
          <View className="flex flex-col space-y-2 px-4">
            <View className="flex flex-row items-center justify-between mb-4">
              <ThemedText className="text-xl font-pbold">Recent activity</ThemedText>
              <Button
                title="Track"
                handlePress={() =>
                  router.push(
            ...
            {isLoading ? (
              <View className="mt-10">
                <ActivityIndicator animating={isLoading} color="#84cc16" size="large" />
              </View>
            ) : isError ? (
              <ThemedText className="text-lg text-center opacity-30 font-pbold">{error.message}</ThemedText>
            ) : activity?.length === 0 ? (
              <ThemedText className="text-lg text-center opacity-30 font-pbold">No activity yet</ThemedText>
            ) : (
              activity?.map((entry) => (
                <ThemedView
                  key={entry.entry_date}
                  className={`flex-row items-center justify-between p-3 rounded-lg border ${
                    colorScheme === 'light' ? 'border-neutral-200' : 'border-neutral-700'
                  }`}
                >
                  <ThemedText className="font-pbold">{entry.status}</ThemedText>

                  <ThemedText className="font-pregular">
                    {entry?.entry_date ? formatRelative(new Date(entry.entry_date), new Date()) : ''}
                  </ThemedText>
                </ThemedView>
              ))
            )}
